#!/usr/bin/env bash
#
# Sign (and, for a .dmg, notarize + staple) a macOS artifact for HyprCorrect.
#
...
#
# Usage:
#   sign-and-notarize.sh --app target/macos/HyprCorrect.app
#       Code-sign the bundle (Developer ID + hardened runtime). No
#       notarization here — the .app is notarized as part of the DMG.
#   sign-and-notarize.sh --dmg target/macos/HyprCorrect-<ver>-aarch64.dmg
...
#   HYPRCORRECT_SIGNING_PASSWORD   app-specific password from
...
: "${HYPRCORRECT_SIGNING_PASSWORD:?app-specific password required for notarytool}"
...
KEYCHAIN_PATH="$WORK_DIR/hyprcorrect-signing.keychain-db"
P12_PATH="$WORK_DIR/hyprcorrect-signing.p12"
...
# No --entitlements: hyprcorrect needs none. Input Monitoring (its
# CGEventTap) and Accessibility (posting synthetic CGEvents) are runtime
# TCC grants, not entitlements. Add the flag here if that ever changes.
...
if [[ "$KIND" == "app" ]]; then
    ...
    # outer wrapper. hyprcorrect ships a single Mach-O today, so the
    # loop is usually empty — keeping it means we don't relearn this
    # when we add a helper.
    ...
    codesign --force --options runtime --timestamp \
        --keychain "$KEYCHAIN_PATH" \
        --sign "$IDENTITY" \
        "$TARGET/Contents/MacOS/hyprcorrect"
...
notary() {
    xcrun notarytool "$@" \
        ...
        --password "$HYPRCORRECT_SIGNING_PASSWORD"
}
